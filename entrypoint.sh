...
# ── Start FastAPI — reads ChromaDB from GCS mount directly ───────────────────
# GCS FUSE handles sequential reads (HNSW index load) fine.
# First query takes ~20s to load index into memory; subsequent queries are fast.
export CHROMA_DB_PATH=/data/chromadb
